...
def train_quantized_model_parameterized(coeff, topology, QW, QB, QA, X_test, Y_test, X_train, Y_train, output_quantizer=1, BATCH_SIZE=100, EPOCH=10):
    ...
    # bitwidth_biases=[sum(QB[0]), sum(QB[1])]
    bitwidth_activation=sum(QA[0])

    max_weights=[2**(QW[0][0]-1)-1, 2**(QW[1][0]-1)-1]
    max_biases=[2**(QW[0][0]-1)-1, 2**(QW[1][0]-1)-1]
    # max_biases=[2**(QB[0][0]-1)-1, 2**(QB[1][0]-1)-1]
    
    # max_weights=max_weights[max_weights==0]=1
    # max_biases=max_biases[max_biases==0]=1
    ...
    # max_biases = list(map(lambda x: x.replace(0, 1), max_biases))

    if output_quantizer==2:
      output_quantizer=quantized_bits(bits=bitwidth_weights[1], 
                                      integer=QW[1][0])
    else:
    ...
    # qb=[(QB[0][0], QB[0][1]), (QB[1][0], QB[1][1])]
...
def train_floating_model(dataset_name, training_flag=False):
  ...
  if dataset_name in ['Arrhythmia']:
      ...
      dataset = Cardio()
      hidden_layer_neurons = 3
  # elif dataset_name in ['Dermatology']:
  #     dataset = Dermatology()
  #     hidden_layer_neurons=3
  # elif dataset_name in ['GasID']:
  #     dataset = GasID()
  #     hidden_layer_neurons = 3
